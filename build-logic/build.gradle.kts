...
val nodeSetupTask = tasks.named<NodeSetupTask>("nodeSetup")
val nodeExecutable = nodeSetupTask.flatMap { it.nodeDir.file(if (windows) "node.exe" else "bin/node") }
val npmExecutable = nodeSetupTask.flatMap { it.nodeDir.file(if (windows) "npm.cmd" else "bin/npm") }
